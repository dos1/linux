...
int rsi_stats_frame(struct rsi_hw *adapter)
{
	...
	redpine_dbg(MGMT_DEBUG_ZONE, "===> Sending PER STATS REQUEST FRAME <===\n");
...
int rsi_mgmt_send_rf_reset_req(struct rsi_hw *adapter, u16 *bb_prog_vals)
{
	...
	redpine_dbg(MGMT_DEBUG_ZONE, "===> Frame request to reset RF <===\n");
	skb        = dev_alloc_skb(FRAME_DESC_SZ);
	mgmt_frame = (struct rsi_mac_frame *)skb->data;

	memset(mgmt_frame, 0, FRAME_DESC_SZ);
	if (bb_prog_vals == NULL) {
		redpine_dbg(ERR_ZONE, "RF_RESET REQUEST NULL PTR\n");
		return -1;
	}
	/* FrameType */
	...
	mgmt_frame->desc_word[0] = cpu_to_le16(RSI_WIFI_MGMT_Q << 12);
	redpine_dbg(INFO_ZONE, " rf_reset: value is 0x%x, rf_delay: %d\n", mgmt_frame->desc_word[3], mgmt_frame->desc_word[4]);
	skb_put(skb, FRAME_DESC_SZ);
	status = rsi_send_internal_mgmt_frame(adapter->priv, skb);
	return status;
}
...
int set_per_configurations(struct rsi_hw *adapter)
{
	...
	if (rsi_load_bootup_params(common) == 0) {
		redpine_dbg(INFO_ZONE, "%s: BOOTUP Parameters loaded successfully\n", __FUNCTION__);
	} else {
		redpine_dbg(ERR_ZONE, "%s: Failed to load bootup parameters\n", __FUNCTION__);
	}
	status = rsi_load_radio_caps(common);
	if (status) {
		redpine_dbg(ERR_ZONE, "%s: Failed to Send Radio parameters\n", __FUNCTION__);
		return status;
	}
	redpine_dbg(INFO_ZONE, "%s: Send Radio parameters\n", __func__);
	if (DEV_MODEL_9116) {
		status = rsi_send_w9116_features(common);
		if (status) {
			redpine_dbg(ERR_ZONE, "%s: Failed to Send wlan_9116 features parameters\n", __FUNCTION__);
			return status;
		}
	}
	status = rsi_program_bb_rf(common);
	redpine_dbg(INFO_ZONE, "%s: bb rf programmed\n", __func__);
	if (status) {
		redpine_dbg(ERR_ZONE, "%s: Failed to SEND bb_rf_init \n", __FUNCTION__);
		return status;
	}
	return status;
}
...
int rsi_send_rx_stats_cmd(struct rsi_hw *adapter, struct nlmsghdr *nlh)
{
	...
	if (adapter->per_params.channel == 0xFF) {
		if (rsi_send_bb_reset_req(adapter) != 0) {
			redpine_dbg(ERR_ZONE, "%s: Failed to send bb reset req\n", __func__);
			return -1;
		}
...
#endif
				redpine_dbg(ERR_ZONE, "%s: Failed to set the channel\n", __func__);
				return -1;
			}
...
SEND_STATS_FRAME:
	...
	int prepare_per_pkt(struct rsi_hw *adapter, struct sk_buff *skb)
	{
		...
		if (adapter->per_params.mode == PER_CONT_MODE) {
			...
			redpine_dbg(ERR_ZONE, "CONTINUOUS PER MODE STARTED\n");
		}
	...
	int do_continuous_send(struct rsi_hw *adapter)
	{
		...
		if (status < 0) {
			redpine_dbg(ERR_ZONE, "Failed to write the packet\n");
			return -1;
		}
	...
	int send_per_ampdu_indication_frame(struct rsi_common *common)
	{
		...
		redpine_dbg(INT_MGMT_ZONE, "<===== SENDING AMPDU_IND FRAME =====>\n");
...
SEND_PER_CMD:
			if ((!adapter->per_params.pkt_length)) {
				redpine_dbg(ERR_ZONE, "Continuous TX len not supported\n");
				return -1;
			}
			if (adapter->per_params.mode == PER_CONT_MODE) {
				redpine_dbg(INFO_ZONE, "Enabling the PER continuous mode\n");
				if (do_continuous_send(adapter)) {
					redpine_dbg(ERR_ZONE, "Can't send do_continuous failed : PER\n");
					return -1;
				}
			...
			if (adapter->per_params.mode == PER_BURST_MODE) {
				...
				if (adapter->per_params.aggr_enable == 1) {
					...
					if (common->fsm_state == FSM_MAC_INIT_DONE) {
						...
						redpine_dbg(ERR_ZONE, "Driver is not in MAC_INIT_DONE state\n");
						return -1;
					}
				}
			}
		...
		if (common->driver_mode == RF_EVAL_MODE_ON) {
			...
			if (adapter->per_params.enable) {
				...
				if (!(common->band == NL80211_BAND_5GHZ)
					...
					redpine_dbg(ERR_ZONE, "Invalid Channel Number for Module type\n");
					return -1;
				}
			}
