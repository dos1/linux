...
static void rsi_nl_recv_msg(struct sk_buff *skb)
{
	...
	if (pkt_type == WLAN_PACKET) {
		switch (cmd) {
		case PER_RECV_STOP:
			redpine_dbg(INFO_ZONE, "%s: stop PER receive case\n", __func__);
			adapter->recv_stop       = 1;
		...
		case PER_RECEIVE:
			...
			if (adapter->priv->fsm_state == FSM_MAC_INIT_DONE) {
				...
				redpine_dbg(MGMT_DEBUG_ZONE, "%s: PER receive case\n", __func__);
				rsi_send_rx_stats_cmd(adapter, nlh);
			} else {
				redpine_dbg(INFO_ZONE, "%s: uninitialized fsm state\n", __func__);
				return;
			}
			break;
		...
		case PER_TRANSMIT:
			if (adapter->priv->fsm_state == FSM_MAC_INIT_DONE) {
				redpine_dbg(MGMT_DEBUG_ZONE, "%s: PER TRANSMIT case\n", __func__);
				payload_len = sizeof(adapter->per_params);
				rsi_transmit_stats_cmd(adapter, nlh, payload_len);
			} else {
				redpine_dbg(ERR_ZONE, "Driver not installed before issuing transmit command\n");
			}
			break;
		case PER_PACKET:
			redpine_dbg(MGMT_DEBUG_ZONE, "%s: PER PACKET case\n", __func__);
			payload_len = sizeof(adapter->per_packet);
			memcpy((&adapter->per_packet), nlmsg_data(nlh) + FRAME_DESC_SZ, payload_len);
			break;
		case RSI_SET_BB_RF:
			adapter->wlan_nl_pid = pid;
			redpine_dbg(MGMT_DEBUG_ZONE, "%s: RSI_SET_BB_RF case\n", __func__);
			...
			if ((rsi_set_bb_rf_values(adapter)) < 0) {
				redpine_dbg(ERR_ZONE, "Invalid Arguments by user\n");
			}
			break;
		case RSI_EFUSE_MAP:
			adapter->wlan_nl_pid = pid;
			redpine_dbg(MGMT_DEBUG_ZONE, "%s: EFUSE MAP case\n", __func__);
			rsi_copy_efuse_content(adapter);
			break;
		default:
			redpine_dbg(ERR_ZONE, "Invalid Message Type\n");
		}
	} else if (pkt_type == BT_PACKET) {
		switch (cmd) {
#if defined(CONFIG_REDPINE_COEX_MODE) || defined(CONFIG_RSI_BT_ALONE)
		case BT_E2E_STAT:
			redpine_dbg(INFO_ZONE, "BT E2E STAT from App\n");
			...
			if (status < 0)
				redpine_dbg(ERR_ZONE, " Failed in BT E2E STAT\n");
			return;
			break;
		case BT_BLE_GAIN_TABLE:
			redpine_dbg(INFO_ZONE, "BT GAIN TABLE UPDATE from App\n");
			adapter->bt_nl_pid = pid;
			payload_len        = nl_desc->desc_word[1];
			rsi_hex_dump(DATA_TX_ZONE, "TX BT pkt", skb->data, skb->len);
			status = rsi_bt_ble_update_gain_table(adapter, nlh, payload_len, cmd);
			if (status < 0)
				redpine_dbg(ERR_ZONE, " Failed in BT/BLE GAIN TABLE UPDATE\n");
			break;
#endif
		default:
			redpine_dbg(ERR_ZONE, "Invalid Message Type\n");
