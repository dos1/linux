...
static int rsi_validate_bgscan_params(struct rsi_common *common, int *bgscan_vals, int user_params_cnt)
{
	...
	if ((bgscan_info->bgscan_threshold < 0) ||
		...
		redpine_dbg(ERR_ZONE, "Bg scan arguments can't be negative vals\n");
		return -EINVAL;
	}
	...
	if (!bgscan_info->active_scan_duration ||
		...
		redpine_dbg(ERR_ZONE, "Active scan duration should be greater"
	...
	if (!bgscan_info->passive_scan_duration ||
		...
		redpine_dbg(ERR_ZONE, "Passive scan duration should be greater"
				" than 0 and less than 256\n");
		return -EINVAL;
	}
	...
	if (!((bgscan_info->two_probe &&
		...
		redpine_dbg(ERR_ZONE, "value for two_probe is either 0 or 1\n");
		return -EINVAL;
	}

	if ((user_params_cnt - 7) != bgscan_vals[6]) {
		redpine_dbg(ERR_ZONE, "channels count is more than"
				  " Provided Channels\n");
		return -EINVAL;
	}
	bgscan_info->debugfs_bg_channels = bgscan_vals[6];
	if (bgscan_info->debugfs_bg_channels > MAX_BG_CHAN_FROM_USER) {
		redpine_dbg(ERR_ZONE, "User provided channels  are more"
	...
	if (rsi_validate_debugfs_bgscan_channels(common)) {
		redpine_dbg(ERR_ZONE, "User provided invalid channels\n");
		return -EINVAL;
	}
	return 0;
}
...

{
  ...
  struct ieee80211_vif *vif = NULL;
	...
	if (common->iface_down) {
		redpine_dbg(ERR_ZONE, "Interface is Down: Must be Up\n");
		return -ENODEV;
	}
	adapter = common->priv;
  ...
  bss = &vif->bss_conf;
	...
	if (rsi_validate_bgscan_params(common, bgscan_vals,
							user_params_cnt)) {
		redpine_dbg(ERR_ZONE, "User provided invalid bgscan parameters\n");
		return -1;
	}
	...
	redpine_dbg(INFO_ZONE, "bgscan params update complete\n");
...
int rsi_validate_ps_params(struct rsi_common *common, int *ps_params_vals)
{
	...
	if ((ps_info->sleep_type != 1) && (ps_info->sleep_type != 2)) {
		redpine_dbg(ERR_ZONE, "Entered wrong value for sleep_type\n");
		redpine_dbg(ERR_ZONE, "Please Enter 1- For LP mode\n 2-ULP Mode\n");
		return -EINVAL;
	}
	...
	if (ps_info->tx_threshold > 10) {
		...
		redpine_dbg(ERR_ZONE, "Supported TX threshold is 0 to 10Mbps\n");
		ps_info->tx_threshold = 1;
	}
	if (ps_info->rx_threshold > 10) {
		redpine_dbg(ERR_ZONE,
			"Making rx threshold  to default value of 1Mbps\n");
		redpine_dbg(ERR_ZONE, "Supported RX threshold is 0 to 10Mbps\n");
	...
	if (ps_info->rx_threshold &&
		...
		redpine_dbg(ERR_ZONE, "Making RX Hysterisis value to RX threshold "
	...
	if (ps_info->tx_threshold &&
		...
		redpine_dbg(ERR_ZONE, "Making TX Hysterisis value to TX threshold "
			"as hysterisis value should not be greater than "
			"tx threshold\n");
		ps_info->tx_hysterisis = ps_info->tx_threshold;
	}
	...
	if ((ps_info->monitor_interval &&
		...
		redpine_dbg(ERR_ZONE, "Supported Monitor Interval Range"
			"is 10 to 30000ms\n");
		redpine_dbg(ERR_ZONE, "Making monitor interval to default value"
	...
	if (ps_info->listen_interval_duration > 65535) {
		...
		redpine_dbg(ERR_ZONE, "Supported Range is 0 to 65535\n");
		return -EINVAL;
	}
	...
	if ((ps_info->num_bcns_per_lis_int > 4095) ||
		...
		redpine_dbg(ERR_ZONE, "Supported Range is 0 to 4095 beacons\n");
		return -EINVAL;
	}
	ps_info->dtim_interval_duration =  ps_params_vals[8];
	if ((ps_info->dtim_interval_duration > 10000) ||
		(ps_info->dtim_interval_duration < 0)) {
		redpine_dbg(ERR_ZONE, "Please enter a valid value for"
				"dtim_interval_duration\n");
		redpine_dbg(ERR_ZONE, "Supported Range is 0 to 10000ms\n");
		return -EINVAL;
	}
	...
	if ((ps_info->num_dtims_per_sleep > 10) ||
		...
		redpine_dbg(ERR_ZONE, "Supported Range is 0 to 10 dtim intervals\n");
		return -EINVAL;
	}
	...
	if ((!ps_info->listen_interval_duration) && (!ps_info->num_bcns_per_lis_int)
		...
		redpine_dbg(ERR_ZONE, "Listen_interval and num_bcns_per_lis_int"
			"and dtim_interval_duration and num_dtims_per_sleep for"
			" are not provided!!\n");
		redpine_dbg(ERR_ZONE, "Wakeup period is set as one dtim!!\n");
	...
	if ((ps_info->uapsd_wakeup_period) &&
		...
		redpine_dbg(ERR_ZONE, "UAPSD wakeup period value should be in"
...
int redpine_init_dbgfs(struct rsi_hw *adapter)
{
	...
	struct redpine_debugfs *dev_dbgfs;
	char devdir[6];
	int ii;
...
{
	struct redpine_debugfs *dev_dbgfs = adapter->dfsentry;
