...
#if defined(CONFIG_REDPINE_COEX_MODE) || defined(CONFIG_REDPINE_BT_ALONE)
#include "rsi_hci.h"
#endif
...
int rsi_send_pkt(struct rsi_common *common, struct sk_buff *skb)
{
	...
	if (!(common->techs[WLAN_ID].tx_intention ||
		...
		status_tx_access = redpine_set_clr_tx_intention(common, COMMON_ID, 1);
		if (status_tx_access) {
...
//#endif
	if (!status_tx_access)
		redpine_set_clr_tx_intention(common, COMMON_ID, 0);
	return status;
}
...
fail:
	return -EINVAL;
		...
		!defined(CONFIG_REDPINE_LOAD_FW_FROM_FLASH_ONLY))
...
static int rsi_load_9116_flash_fw(struct rsi_hw *adapter)
{
	...
	if ((bl_cmd(adapter, LOAD_HOSTED_FW, LOADING_INITIATED,
		    "LOAD_HOSTED_FW")) < 0)
		return -EIO;
	redpine_dbg(ERR_ZONE, "***** Loaded Firmware *****\n");
	return 0;
...
static int rsi_load_9116_firmware_to_ram(struct rsi_hw *adapter, const struct firmware *fw_entry)
{
  ...
  common->lmac_ver.patch_id = (u16)(version_info->patch_num);
	...
	/* memcpy to boot descriptor from TA firmware len should be assign */
		...
		do {
			redpine_dbg(INIT_ZONE, "%s: Loading chunk %d\n",
				__func__, ii);

			redpine_dbg(INIT_ZONE, "length %d: destination %x\n",
				(bootload_ds.bl_entry[ii].control &
				RSI_BL_CTRL_LEN_MASK),
				bootload_ds.bl_entry[ii].dst_addr);

			redpine_dbg(INIT_ZONE, "FW start %x\n",
				*(u32 *)firmware_ptr);
...
#ifdef CONFIG_RS9116_FLASH_MODE
#ifdef CONFIG_REDPINE_LOAD_FW_FROM_FLASH_ONLY
...
upgrade_9116_flash_fw:
		...
		if (bl_cmd(adapter, BURN_HOSTED_FW, SEND_RPS_FILE,
			   "FW_UPGRADE") < 0)
			goto fail;
		redpine_dbg(ERR_ZONE, " Burn Command Pass.. Upgrading the firmware\n");
...
int rsi_validate_oper_mode(u16 oper_mode)
{
	...
	case 1:
		...
		redpine_dbg(ERR_ZONE, "Operating mode %d not supported with"
	...
	case 8:
	case 12:
#ifndef CONFIG_REDPINE_BT_ALONE
		redpine_dbg(ERR_ZONE, "Operating mode %d not supported without"
				  "build flag 'CONFIG_REDPINE_BT_ALONE enabled'\n",
				  oper_mode);
		return -EINVAL;
#else
	...
	case 48:
		...
		return -EINVAL;
#else
#if defined(CONFIG_REDPINE_MULTI_MODE)
		redpine_dbg(ERR_ZONE, "Operating mode %d not supported with build"
			" flags 'CONFIG_REDPINE_MULTI_MODE'\n", oper_mode);
		return -EINVAL;
#endif
		return 0;
#endif
